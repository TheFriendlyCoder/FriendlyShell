...
that are prefixed with the characters 'do_' (ie: 'do_exit()'). Because of this
commands must meet the same naming restrictions as Python class methods. Thus
most Friendly Shells line parsers will want to leverage the :call:`meth`
...
def default_command_token():
    ...
    The resulting token will be labelled 'commmand' as required by the
    Friendly Shell APIs.
...
def quoted_space_sep_params_token():
    ...
    the same name but with a prefix of 'do_'. As a result tokens for commands
