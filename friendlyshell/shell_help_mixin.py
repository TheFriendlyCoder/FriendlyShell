...
class ShellHelpMixin(object):
    ...
    def _list_commands(self):
        ...
        for cur_method in all_methods:
            ...
            # the name of the method and the second a reference to the method object
            ...
            if method_name.startswith('do_'):
                ...
                if hasattr(self, method_name.replace('do_', 'help_')):
                    # NOTE:
                    # For the sake of online help, we'll assume that class attributes with 'help_' in
                    # their name are methods which can be called to display verbose help. We can add
                    # verification logic for this elsewhere when necessary
                    command_list['Extended Help'].append('`' + self.prompt + 'help ' + cmd_name + '`')
                else:
    ...
    def do_help(self, arg=None):
        ...
        :param str arg: Optional command to generate online help for. If not defined, show a list of commands"""
        if arg is None:
            self._list_commands()
            return
        ...
        if not hasattr(self, method_name):
            ...
            if hasattr(self, cmd_method_name):
                self._log.info(self._log.getattr(self, 'do_' + arg).__doc__.split('\n')[0])
            return

        func = getattr(self, method_name)
        if not inspect.ismethod(func):
            self._log.error('Error: definition "%s" in derived class must be a method. Check implementation',
                            method_name)
            return
    ...
    def complete_help(self, parser, parameter_index, cursor_position):
        ...
        return self._complete_command_names(parser.params[parameter_index][:cursor_position])

    def help_help(self):
        """Generates inline help for the 'help' command"""
        retval = [
            "Online help generation tool",
            "Running 'help' with no parameters displays a list of supported commands",
            "Passing any supported command to 'help' provides detailed help on the command",
